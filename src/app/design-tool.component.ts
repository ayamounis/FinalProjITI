...
import { CustomProductRequest, ProductTemplateRequest } from './models/product.model';
...
export class DesignToolComponent implements AfterViewInit {
  ...
  loadTemplate(template: ProductsTemplatesResponse) {
    // Confirm before loading a new template if there are objects on the canvas
    ...
    setTimeout(() => {
      ...
      fabric.Image.fromURL(pngUrl, (img) => {
        ...
        // Define print area based on template
  ...
  async saveDesign() {
    // Saves the current design as a custom product or template
    const canvas = this.canvas();
    const template = this.selectedTemplate();
    ...
    try {
      ...
      const elements = JSON.stringify(canvas.toJSON());

      if (this.isUser()) {
        // Save as custom product for normal users
        ...
        this.productService.createCustomProduct(customProduct).subscribe({
          next: (response) => {
            if (response) {
              this.cartService.addToCart({
                ...
                error: (error) => {
                  ...
                  this.saveMessage.set('Design saved but failed to add to cart');
                  this.isSaving.set(false);
                  setTimeout(() => this.saveMessage.set(''), 3000);
                }
              });
          ...
          error: (error) => {
            ...
            this.saveMessage.set('Failed to save design. Please try again.');
            this.isSaving.set(false);
            setTimeout(() => this.saveMessage.set(''), 3000);
          }
        });
        ...
        this.productService.createTemplateFromDesign(productTemplate).subscribe({
          next: () => {
          ...
          error: (error) => {
            ...
            this.saveMessage.set('Failed to save template. Please try again.');
            this.isSaving.set(false);
            setTimeout(() => this.saveMessage.set(''), 3000);
          }
        });
      ...
      this.saveMessage.set('An unexpected error occurred');
      this.isSaving.set(false);
      setTimeout(() => this.saveMessage.set(''), 3000);
    }
  }
